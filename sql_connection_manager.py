            ...
            with self._engine.begin() as conn:
                ...
                for tname, fkcs in reversed(
                    ...
                    if fkcs:
                        ...
                        for fkc in fkcs:
                            try:
                                fk_constraint = ForeignKeyConstraint((), (), name=fkc[1])
                                self.logger().info(f'dropping {fkc[1]}')
                                Table(tname, MetaData(), fk_constraint)
                                conn.execute(DropConstraint(fk_constraint))
                            except Exception as e:
                                self.logger().error(f"Error dropping foreign key constraint {fkc[0]} on table {tname}: {e}")
                                continue
